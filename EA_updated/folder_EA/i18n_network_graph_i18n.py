...
def setup_translation(lang):  # lang = "fr" or "de"
    output_lang = f"en-{lang}"  # becomes "en-fr" or "en-de"
    translation = gettext.translation(
        "messages",              
        localedir=LOCALE_PATH,   
        languages=[output_lang], 
        fallback=True            # fallback to English
    )
    translation.install()
...
def bokeh_graph(bms_file_name):
    ...
    for map_id, map_name in map_data.items():
        ...
        for relation in program_data.get("relations", []):
            ...
            for item in relation["data"]:
                ...
                text = text + _("\n* Name: {file_name_unique}").format(file_name_unique=file_name_unique)
    ...
    if generated_files:
        ...
        for file in generated_files:
            ...
            #print(_(f"{file} → Open in browser: {file_url}"))
            print(_("{file} → Open in browser: {file_url}").format(file=file, file_url=file_url))
            webbrowser.open(file_url)
    else:
        print(_(" No graphs were generated."))
...
user_input = "Bonjour"  # French input
detected_lang = detect_language(user_input)
_ = setup_translation(detected_lang)
...
#  python3 /Users/thrisham/Desktop/cobol_code/EA_updated/folder_EA/i18n_network_graph_i18n.py